...
class TrackingLot(models.Model):
    ...
    parent_lot_id = fields.Many2one('erpv6.tracking.lot', string='Lotto Batch Padre', domain=[('tracking_type', '=', 'batch')])
    child_lot_ids = fields.One2many('erpv6.tracking.lot', 'parent_lot_id', string='Lotti Figli')
    config_id = fields.Many2one('erpv6.tracking.config', string='Configurazione', required=True)
    
    company_code = fields.Char('Sigla Azienda', required=True)
    brand_code = fields.Char('Codice Brand', required=True)
    
    production_date = fields.Datetime('Data Produzione', required=True, default=fields.Datetime.now)
    ...
    state = fields.Selection([
        ('draft', 'Bozza'), ('active', 'Attivo'), ('closed', 'Chiuso'), ('cancelled', 'Annullato'),
    ], string='Stato', default='draft', required=True)

    _sql_constraints = [('code_unique', 'unique(code)', 'Il codice lotto deve essere univoco!')]

    @api.model
    def _get_julian_day(self, date=None):
        if not date: date = fields.Datetime.now()
        if isinstance(date, str): date = datetime.strptime(date, '%Y-%m-%d %H:%M:%S')
        return date.timetuple().tm_yday

    @api.model
    def _generate_batch_code(self, config, production_date=None):
        if not production_date: production_date = fields.Datetime.now()
        if isinstance(production_date, str): production_date = datetime.strptime(production_date, '%Y-%m-%d %H:%M:%S')
        year = str(production_date.year)[-2:]
        julian_day = self._get_julian_day(production_date)
        # FIX SICUREZZA: Usato secrets al posto di random
    ...
    def create_batch_lot(self, config_id, product_id=None, quantity=1.0, notes=None):
        ...
        return self.create({
            'name': f'Lotto Batch {code}', 'code': code, 'tracking_type': 'batch', 'config_id': config.id,
            'company_code': config.company_code, 'brand_code': config.default_brand, 'production_date': production_date,
            'year': str(production_date.year), 'julian_day': self._get_julian_day(production_date),
            'hour': production_date.hour, 'minute': production_date.minute, 'product_id': product_id,
            'quantity': quantity, 'notes': notes, 'state': 'active',
        })
    ...
    def create_definitive_lot(self, config_id, batch_lot_id=None, product_id=None, quantity=1.0, notes=None):
        ...
        # Gestione duplicati codice
        counter = 1
        while self.search([('code', '=', f"{code}-{counter:02d}")], limit=1):
            counter += 1
        if counter > 1: code = f"{code}-{counter:02d}"
        
        return self.create({
            'name': f'Lotto Definitivo {code}', 'code': code, 'tracking_type': 'definitive', 'parent_lot_id': batch_lot_id,
            'config_id': config.id, 'company_code': config.company_code, 'brand_code': config.default_brand,
            'production_date': production_date, 'year': str(production_date.year),
            'julian_day': self._get_julian_day(production_date), 'hour': production_date.hour, 'minute': production_date.minute,
            'product_id': product_id, 'quantity': quantity, 'notes': notes, 'state': 'active',
        })
