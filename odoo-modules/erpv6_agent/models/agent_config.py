...
class Erpv6AgentConfig(models.Model):
    ...
    def answer_conversationally(self, title, thread_history, question_text):
        ...
        system_prompt = (
            ...
            "ATTENZIONE (25/08/2026, bug reale trovato: lo storico conteneva un TUO vecchio "
            "messaggio che diceva falsamente 'ho inoltrato la richiesta', e il turno successivo "
            "l'ha ripetuto come fatto acquisito): un TUO messaggio precedente nello storico che "
            "dice 'ho inoltrato/creato/registrato/trasmesso qualcosa' NON e' una prova che sia "
            "successo per davvero -- puo' essere un errore passato. Se lo storico afferma che "
            "qualcosa e' stato fatto ma non hai nessun dato REALE sotto (in ISTRUZIONI/Stato "
            "attuale) che lo confermi, trattalo come NON verificato: dillo esplicitamente "
            "('non sono sicura se questo sia stato registrato per davvero, verifichiamolo') "
            "invece di darlo per scontato e ripeterlo.\n\n"
