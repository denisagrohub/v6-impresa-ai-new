...
class Erpv6CoreNodeExecution(models.Model):
    ...
    # Denis, 30/08/2026, prompt #11 -- solo schema, nessuna generazione/
    # collegamento qui (Parte 2, separata). Char (non Integer): stessa
    # rappresentazione di erpv6.core.gate.log.giro_id (li' gia' dichiarato
    # Char nello schema del prompt), pensato come UUID quando la Parte 2
    # decidera' dove generarlo -- nessun identificatore di riga/batch
    # generico riusabile trovato altrove nel progetto (erpv6.core.circuit.
    # run.id esiste ma e' agganciato solo al percorso 6 Giudici via
    # validation_session_id, non al percorso generico run_process()/
    # run_circuit() che questo campo deve coprire). Resta vuoto finche' non
    # scritto esplicitamente -- nessun default, nessuna generazione qui.
    giro_id = fields.Char(index=True)
