...
class Erpv6CoreOutput(models.Model):
    ...
    # Denis, 30/08/2026, Admin Visual V4 (cross-reference) -- stesso pattern
    # gia' reale di erpv6.typst.template.usage_count/smart-button (conta
    # erpv6.typst.document collegati), riusato identico: qui conta quanti
    # erpv6.core.output_link (nodi consumatori) puntano a questo Output.
    consumer_count = fields.Integer(compute='_compute_consumer_count')

    @api.depends()
    def _compute_consumer_count(self):
        OutputLink = self.env['erpv6.core.output_link']
        for rec in self:
            rec.consumer_count = OutputLink.search_count([('output_id', '=', rec.id)])

    def action_view_consumers(self):
        """Bottone smart button. Nota (Denis, 30/08/2026): il pattern trovato
        in Fase 0 (erpv6.typst.template, button type='action' su un xmlid
        statico) NON filtra davvero per il record sorgente -- verificato
        leggendo action_erpv6_typst_document, nessun domain sul
        template_id. Stesso aspetto visivo (oe_stat_button/statinfo), ma
        qui uso type='object' + azione dinamica con domain esplicito, cosi'
        il cross-reference filtra DAVVERO."""
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
            'name': _('Nodi consumatori — %s') % self.name,
            'res_model': 'erpv6.core.output_link',
            'view_mode': 'list,form',
