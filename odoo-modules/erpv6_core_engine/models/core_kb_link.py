...
class Erpv6CoreKbLink(models.Model):
    ...
    # Denis, 30/08/2026, Admin Visual V4 (cross-reference) -- un kb_link e'
    # gia' 1:1 col proprio target_node_id (visibile direttamente sul
    # record, nessun cross-reference serve li'). Il cross-reference utile e'
    # un altro: "chi ALTRO nel grafo dipende dalla STESSA conoscenza" --
    # altri kb_link che puntano allo stesso kb_id (fixed_kb) o allo stesso
    # kb_type_dynamic (dynamic), ciascuno con il proprio target_node_id
    # visibile in lista -- risponde indirettamente a "quali nodi" elencando
    # i rombi gemelli e il nodo di ognuno.
    other_consumers_count = fields.Integer(compute='_compute_other_consumers_count')

    @api.depends('resolution_mode', 'kb_id', 'kb_type_dynamic')
    def _compute_other_consumers_count(self):
        for rec in self:
            if rec.resolution_mode == 'fixed_kb' and rec.kb_id:
                rec.other_consumers_count = self.search_count([
                    ('id', '!=', rec.id), ('resolution_mode', '=', 'fixed_kb'), ('kb_id', '=', rec.kb_id.id),
                ])
            elif rec.resolution_mode == 'dynamic' and rec.kb_type_dynamic:
                rec.other_consumers_count = self.search_count([
                    ('id', '!=', rec.id), ('resolution_mode', '=', 'dynamic'),
                    ('kb_type_dynamic', '=', rec.kb_type_dynamic),
                ])
            else:
                rec.other_consumers_count = 0

    def action_view_other_consumers(self):
        self.ensure_one()
        if self.resolution_mode == 'fixed_kb' and self.kb_id:
            domain = [('id', '!=', self.id), ('resolution_mode', '=', 'fixed_kb'), ('kb_id', '=', self.kb_id.id)]
        elif self.resolution_mode == 'dynamic' and self.kb_type_dynamic:
            domain = [
                ('id', '!=', self.id), ('resolution_mode', '=', 'dynamic'),
                ('kb_type_dynamic', '=', self.kb_type_dynamic),
            ]
        else:
            domain = [('id', '=', -1)]
        return {
            'type': 'ir.actions.act_window',
            'name': _('Altri rombi sulla stessa conoscenza'),
            'res_model': 'erpv6.core.kb_link',
            'view_mode': 'list,form',
            'domain': domain,
