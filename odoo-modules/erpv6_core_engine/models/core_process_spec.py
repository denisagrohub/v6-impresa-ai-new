...
from .core_node import TIPO_DATO_VOCABOLARIO, _all_process_key_choices
...
#
# Denis, 30/08/2026, prompt #21: process_key qui sotto era una Selection
# STATICA (_PROCESS_KEY_SELECTION, lista congelata all'import, basata
# SOLO sul dizionario locale SAFE_PROCESSES di core_node.py) -- stesso
# identico bug gia' corretto per erpv6.core.node.process_key nel prompt
# #15 (selection=callable), segnalato allora come gap noto qui e mai
# corretto insieme. Un Motore registrato SOLO via erpv6_core_dispatch
# (mai nel dizionario locale, es. disc_interview_score) lo ha reso
# bloccante per la prima volta -- ValueError "Wrong value for erpv6.core.
# process.input_spec.process_key" al momento di dichiarare la sua firma.
# Corretto ora con la STESSA funzione condivisa (_all_process_key_choices,
# estratta da core_node.py), non una copia della logica di merge.
...
class Erpv6CoreProcessInputSpec(models.Model):
    ...
    process_key = fields.Selection(
        selection='_selection_process_key', required=True,
        help="Elenco chiuso -- stessi valori di SAFE_PROCESSES/cron_trigger_key, mai un nome "
             ...
             "propria. DINAMICO (prompt #21) -- vedi Erpv6CoreNode._selection_process_key.")
...
class Erpv6CoreProcessOutputSpec(models.Model):
    ...
    process_key = fields.Selection(
        selection='_selection_process_key', required=True,
        help="DINAMICO (prompt #21) -- vedi Erpv6CoreNode._selection_process_key.")
