...
class Erpv6CoreEngineController(http.Controller):
    ...
    def get_circuit(self, xmlid, **kwargs):
        ...
        nodes = [{
            ...
            'last_execution': (lambda ex: {
                ...
                # Denis, 30/08/2026, prompt #6/#5: morsettiera -- diagnostico
                # gia' sul record (compute store=True), solo esposto qui,
                # nessun calcolo nuovo lato controller.
                'firma_soddisfatta': ex.firma_soddisfatta, 'firma_mancanti': ex.firma_mancanti or False,
            } if ex else False)(
        ...
        output_link_data = [{
            ...
            # Denis, 30/08/2026, prompt #3: stesso principio gia' esposto
            # sopra per kb_link_data.format_mismatch, esteso all'altra
            # fonte reale di input di un Motore (Output-Link).
            'format_mismatch': l.format_mismatch,
    ...
    def processes(self, **kwargs):
        ...
        Denis, 30/08/2026, prompt #4/#7: aggiunge input_spec/output_spec
        per process_key -- la firma dichiarativa (erpv6.core.process.
        input_spec/.output_spec), letta da qui, non ricalcolata lato
        frontend. kb_engine_process ha SEMPRE input_spec=[] per costruzione
        (caso limite: firma variabile per kb_type, resta in
        KB_ENGINE_REQUIRED_INPUTS dentro _run_kb_engine_process, mai
        duplicata qui -- vedi core_node.py)."""
        env = request.env(su=True)
        InputSpec = env['erpv6.core.process.input_spec']
        OutputSpec = env['erpv6.core.process.output_spec']
        result = []
        for k, v in SAFE_PROCESSES.items():
            input_spec = InputSpec.search([('process_key', '=', k)], order='obbligatorio desc, input_key')
            output_spec = OutputSpec.search([('process_key', '=', k)], limit=1)
            result.append({
                'key': k, 'label': v['label'], 'family': v['family'],
                'input_spec': [{
                    'input_key': s.input_key, 'tipo_dato': s.tipo_dato, 'obbligatorio': s.obbligatorio,
                    'note': s.note or False, 'tag_dominio': s.tag_dominio or False,
                } for s in input_spec],
                'output_spec': {
                    'output_tipo_dato': output_spec.output_tipo_dato, 'tag_dominio': output_spec.tag_dominio or False,
                } if output_spec else False,
            })
