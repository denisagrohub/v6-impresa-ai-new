...
export default function GraphTestPublicPage() {
  ...
  const syncToNeo4j = async () => {
    ...
    try {
      ...
      console.error(err);
      alert("Errore di connessione API Neo4j");
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadFromNeo4j = async () => {
    ...
    try {
      ...
      if (data.nodes) {
        ...
        alert("Nessun dato restituito da Neo4j");
      }
    } catch (err) {
      console.error(err);
        ...
        <header className="flex items-center justify-between border-b border-zinc-800 pb-4">
          <div>
            <h1 className="flex items-center gap-2.5 text-xl font-semibold tracking-tight">
              ...
              Grafo DAG Adaptive EOSv6 (Vista Pubblica di Test)
            </h1>
            <p className="mt-0.5 text-xs text-zinc-500">
              Pagina temporanea slegata dal Login per la verifica locale.
            </p>
          </div>
